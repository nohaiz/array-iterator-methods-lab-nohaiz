...
veryOldInventors.push(inventors.filter((person) => person.year > 1500 && person.year < 1600));
...
inventorNames.push(inventors.map((personName) => ({first: personName.first, last: personName.last})));
...
firstLast.push((people.map((person) => person.split(', ').reverse().join(' '))));
...
devs.some((dev) => {
    if ((currentYear - dev.year) >= 18 )
    {
        return isAdultPresent = true
    }
});
...
devs.every((dev) => {
    if ((currentYear - dev.year) >= 19 )
    {
        return isEveryone19OrOlder = false
    }
});
...
commentById = comments.find((commentId) => commentId.id === 823423);
...
idx = comments.findIndex((commentIdex) => commentIdex.id === 123523);
...
function ageCalculator(startYear,endYear) {
    return endYear - startYear;
}
// Enter your solution code here:

totalYearsLived = inventors.reduce((sum ,inventor) => {

    let age = ageCalculator(inventor.year,inventor.passed);
    return sum + age
}, 0);
...
travelMethodCounts = travelMethods.reduce((sum,travel) => {
    if (sum.hasOwnProperty(travel)) {
        sum[travel] ++;
    }
    else {
        sum[travel] = 1;        
